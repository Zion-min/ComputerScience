...
int main(int argc, char* argv[])
{
    ...
    while(1)
    {
//        udp다룰때는, 일단 최대한 꽉꽉 눌러담아서 buf size만큼 한번에 받아옴. 그리고 나서! 먼가 client에서 줬던거처럼 세심하게 골라 담아 됨. 3번째 인자에 (opCount*5) 넣었다가 자꾸 이상한거 나와서 식겁침 ... 
        ...
        {
            operand[i] = ((buf[1+((i*4)+3)] << 24)&0xff000000) | ((buf[1+((i*4)+2)] << 16)&0x00ff0000) | ((buf[1+((i*4)+1)] << 8)&0x0000ff00) | (buf[1+(i*4)]&0x000000ff);
            printf("Operand %d: %d\n", i, operand[i]);
        }
